...
import { encryptTotpSecret } from "./encryptionHelpers";
...
export async function uploadImage(formData: FormData): Promise<string> {
	...
	const result = await userService.uploadProfilePicture(files, userId);
...
export async function updateUser(data: {
	...
	try {
		...
		await userService.updateProfile(userId, servicePayload);
	} catch (error) {
