...
export function makeLocalDate(dateStr: string, timeStr: string) {
	const [y, m, d] = dateStr.split("-").map(Number); // "2025-09-04"
	const [hh, mm = 0, ss = 0] = timeStr.split(":").map(Number); // "15:00"
	// This constructor treats values as LOCAL time (no timezone shift)
