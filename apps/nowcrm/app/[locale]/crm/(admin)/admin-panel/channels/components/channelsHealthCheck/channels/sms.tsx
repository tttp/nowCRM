...
export function SmsHealthCheck({ sns_credential }: LinkedInHealthCheckProps) {
	...
	async function handleSubmit(values: z.infer<typeof formSchema>) {
		...
		try {
			const res = await updateSettingCredentials(sns_credential.documentId, {
				...values,
		...
		<Dialog open={isOpen} onOpenChange={setIsOpen}>
					...
					<CardContent className="p-6">
						<div className="flex items-center justify-between">
							...
							<div className="flex items-center gap-2">
								<span
									className={`rounded-full px-2 py-1 text-xs ${getStatusColor(sns_credential.credential_status)}`}
								>
									{sns_credential.credential_status}
								</span>
								{getStatusIcon(sns_credential.credential_status)}
								<div>
									<TooltipProvider>
										<Tooltip>
			...
			<DialogContent className="min-w-max">
				...
				<div className="py-4">
					...
					{sns_credential?.credential_status === "invalid" ||
					(sns_credential?.credential_status === "disconnected" &&
