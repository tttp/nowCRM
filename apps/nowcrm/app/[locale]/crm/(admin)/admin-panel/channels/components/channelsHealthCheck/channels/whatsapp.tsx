...
export function WhatsAppHealthCheck({
	...
	const formSchema = z.object({
		access_token: z.string().min(1, t.whatsApp.form.accessToken.errorMessage),
		client_id: z.string().min(1, t.whatsApp.form.clientId.errorMessage), // for user uts business account id
	});

	const form = useForm<z.infer<typeof formSchema>>({
		resolver: zodResolver(formSchema),
		defaultValues: whatsapp_credential
			? {
					access_token: whatsapp_credential.access_token,
					client_id: whatsapp_credential.client_id,
				}
			: {
					access_token: "",
					client_id: "",
	...
	async function handleSubmit(values: z.infer<typeof formSchema>) {
		...
		try {
			...
			if (res.success) {
				...
				toast.error(t.whatsApp.toast.error.updateCredentialsError);
			}
			setIsOpen(false);
		} catch (error) {
			toast.error(t.whatsApp.toast.error.failedCredentialsError);
			console.error(error);
		} finally {
												...
												<div className="space-y-2 text-black dark:text-white">
													...
													<div className="mt-2 border-gray-200 border-t pt-2 dark:border-gray-700">
														<div className="flex items-center gap-2 text-muted-foreground text-xs">
															...
															<span>{t.whatsApp.tooltip.price}</span>
														</div>
														<div className="mt-1 flex items-center gap-2 text-muted-foreground text-xs">
															<AlertCircle className="h-3.5 w-3.5" />
															<span>{t.whatsApp.tooltip.info}</span>
														</div>
													</div>
												</div>
											</TooltipContent>
							...
							<FormField
								...
								render={({ field }) => (
									<FormItem>
										...
										<FormControl>
											<Input
												placeholder={t.whatsApp.form.accessToken.placeholder}
												{...field}
											/>
										</FormControl>
										<FormMessage />
							...
							<FormField
								...
								render={({ field }) => (
									<FormItem>
										...
										<FormControl>
											<Input
												placeholder={t.whatsApp.form.clientId.placeholder}
												{...field}
											/>
										</FormControl>
										<FormMessage />
