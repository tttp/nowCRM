...
export function TelegramHealthCheck({
	...
	const formSchema = z.object({
		access_token: z.string().min(1, t.telegram.form.accessToken.errorMessage),
		organization_urn: z
			.string()
			.min(1, t.telegram.form.organizationUrn.errorMessage),
	});

	const form = useForm<z.infer<typeof formSchema>>({
		resolver: zodResolver(formSchema),
		defaultValues: telegram_credential
			? {
					access_token: telegram_credential.access_token,
					organization_urn: telegram_credential.organization_urn,
				}
			: {
					access_token: "",
					organization_urn: "",
	...
	async function handleSubmit(values: z.infer<typeof formSchema>) {
		...
		try {
			const res = await updateSettingCredentials(telegram_credential.id, {
				...values,
				status: "disconnected",
			...
			if (res.success) {
				...
				toast.error(t.telegram.toast.error.updateCredentialsError);
			}
			setIsOpen(false);
		} catch (error) {
			toast.error(t.telegram.toast.error.failedCredentialsError);
			console.error(error);
		} finally {
		...
		<Dialog open={isOpen} onOpenChange={setIsOpen}>
					...
					<CardContent className="p-6">
						<div className="flex items-center justify-between">
							...
							<div className="flex items-center gap-2">
								<span
									className={`rounded-full px-2 py-1 text-xs ${getStatusColor(telegram_credential.status)}`}
								>
									{telegram_credential.status}
								</span>
								{getStatusIcon(telegram_credential.status)}
								<div>
									<TooltipProvider>
										<Tooltip>
												...
												<div className="space-y-2 text-black dark:text-white">
													<div className="flex items-center gap-2">
														...
														<h4 className="font-medium">
															{t.telegram.tooltip.title}
														</h4>
													</div>
													<p className="text-sm">
														{t.telegram.tooltip.description}
													</p>
													<div className="mt-2 border-gray-200 border-t pt-2 dark:border-gray-700">
														<div className="flex items-center gap-2 text-muted-foreground text-xs">
															<DollarSign className="h-3.5 w-3.5" />
															<span>{t.telegram.tooltip.price}</span>
														</div>
														<div className="mt-1 flex items-center gap-2 text-muted-foreground text-xs">
															<AlertCircle className="h-3.5 w-3.5" />
															<span>{t.telegram.tooltip.info}</span>
														</div>
													</div>
												</div>
											</TooltipContent>
			...
			<DialogContent className="min-w-max">
				<DialogHeader>
					...
					<DialogDescription>{t.telegram.description}</DialogDescription>
				</DialogHeader>
				...
				<div className="py-4">
					...
					{telegram_credential?.status === "invalid" ||
					(telegram_credential?.status === "disconnected" &&
						telegram_credential?.error_message) ? (
						<div
					...
					<Form {...form}>
						...
						>
							<FormField
								...
								render={({ field }) => (
									<FormItem>
										<FormLabel>Telegram Bot API token</FormLabel>
										<FormControl>
											<Input
												placeholder={t.telegram.form.accessToken.placeholder}
												{...field}
											/>
										</FormControl>
										<FormMessage />
								...
								render={({ field }) => (
									<FormItem>
												...
												<Tooltip>
													...
													<TooltipContent className="w-80 p-2">
														<p className="mb-1 font-medium">
															{t.telegram.form.organizationUrn.tooltip}
														</p>
													</TooltipContent>
												</Tooltip>
											</TooltipProvider>
										...
										<FormControl>
											<Input
												placeholder={
													t.telegram.form.organizationUrn.placeholder
												}
												{...field}
											/>
										</FormControl>
