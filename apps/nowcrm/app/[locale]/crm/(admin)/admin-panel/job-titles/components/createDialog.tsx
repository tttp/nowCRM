...
export default function CreateJobTitleDialog() {
	...
	async function onSubmit(values: z.infer<typeof formSchema>) {
		...
		const res = await createJobTitle(values.name);
		if (!res.success) {
			toast.error(`${t.Admin.JobTitle.toast.createError}: ${res.errorMessage}`);
		} else {
			toast.success(
				`${t.Admin.JobTitle.toast.jobTitle} ${values.name} ${t.common.actions.created}`,
			);
			router.refresh();
			setDialogOpen(false);
		}
	...
	return (
		<Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
			...
			<DialogContent className="sm:max-w-[425px]">
				<DialogHeader>
					<DialogTitle>{t.Admin.JobTitle.dialog.title}</DialogTitle>
					<DialogDescription>
						{t.Admin.JobTitle.dialog.description}
					</DialogDescription>
				</DialogHeader>
				...
				<Form {...form}>
					<form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 ">
						...
						<Button type="submit" className="w-full cursor-pointer">
							...
							{t.Admin.JobTitle.action.create}
						</Button>
					</form>
				</Form>
			</DialogContent>
