...
export function CSVMappingModal({
	...
	const validateMappings = (
		...
		// Check if any field is mapped to the "name" template field
		const hasNameMapping = Object.entries(currentMappings).some(
			([csvField, mappedValue]) =>
				!currentDeletedColumns.includes(csvField) &&
				mappedValue.toLowerCase() === "name",
		);

		if (!hasNameMapping) {
			errors.push(
				"At least one field must be mapped to the 'name' template field.",
			);
		}
	...
	const hasNameMapping = Object.entries(mappings).some(
		([csvField, templateField]) =>
			!deletedColumns.includes(csvField) &&
			templateField.toLowerCase() === "name",
	);

	// Check if this field has a duplicate mapping
			...
			<DialogContent className="max-h-[80vh] overflow-y-auto sm:max-w-[800px]">
									...
									>
										...
										<SelectContent>
											{contactCSVTemplateFields.map((field) => (
												...
												>
													{field}{" "}
													{field.toLowerCase() === "name" && "(required)"}
												</SelectItem>
											))}
										</SelectContent>
									</Select>
				...
				<DialogFooter className="mt-4 flex justify-between sm:justify-between">
					<Button
						...
						disabled={validationErrors.length > 0 || !hasNameMapping}
					>
						Configure
					</Button>
				</DialogFooter>
