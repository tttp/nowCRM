...
const stepConfig = {
	...
	[STEPS.PREVIEW]: {
		...
		description: "Review filtered contacts",
		icon: Eye,
	},
	[STEPS.UPDATE_SUBSCRIPTION]: {
		title: "Update Subscription",
		description: "Pick channel and action",
		icon: CheckCircle,
	},
};

export default function UpdateSubscriptionWithFiltersStepsDialog({
	selectedOption,
	setSelectedOption,
	setFilters,
	closeDialog,
	...
	setDialogNeeded: (value: boolean) => void;
}) {
	...
	const handlePreviewApprove = () => {
		...
		setCurrentStep(STEPS.UPDATE_SUBSCRIPTION);
		markStepCompleted(STEPS.PREVIEW);
					...
					<React.Fragment key={step}>
						<div className="flex flex-col items-center">
							...
							<div className="mt-2 text-center">
								<div
									className={`font-medium text-sm ${isActive ? "text-blue-600" : isCompleted ? "text-green-600" : "text-gray-500"}`}
								>
									{config.title}
								</div>
	...
	const handleFormSubmitted = (_values: UpdateSubscriptionFormValues) => {
		// hook your mass action trigger here if needed
	};

	return (
		<div>
			<DialogHeader>
				<DialogTitle>Update Subscription for Selected Contacts</DialogTitle>
				<DialogDescription>
					Update subscription for filtered contacts
				</DialogDescription>
			</DialogHeader>
			...
			{currentStep === STEPS.FILTERS && (
				<AdvancedFilters
					key="mass-update-subscription-filters"
					showTrigger={false}
					mode="mass-action"
					onClose={closeDialog}
			...
			{showMainContent && currentStep === STEPS.UPDATE_SUBSCRIPTION && (
				<div className="space-y-4">
					<div className="flex items-center justify-between">
						<h3 className="font-semibold text-lg">Choose Channel and Action</h3>
						<Button
							type="button"
							variant="outline"
							size="sm"
