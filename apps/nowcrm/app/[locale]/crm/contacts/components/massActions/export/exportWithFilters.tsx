...
const stepConfig = {
	...
	[STEPS.PREVIEW]: {
		...
		description: "Review contacts before exporting",
		icon: Eye,
		number: 2,
	},
};

export default function ExportWithFiltersDialog({
	filters,
	setFilters,
	closeDialog,
	onSubmit,
		...
		try {
			if (onSubmit) {
				...
				if (result.success) {
					...
					toast.error(result.errorMessage || "Error exporting contacts");
				}
			}
		} catch (e) {
			console.error("Error in handleConfirmExport:", e);
		} finally {
			closeDialog(false);
		}
	};
	...
	return (
		<div>
			<DialogHeader>
				...
				<DialogDescription>
					This will export <b>all {totalCount} contacts</b> matching your
					filters.
				</DialogDescription>
			</DialogHeader>
